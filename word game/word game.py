...
def isValidWord(word, hand, wordList):
    ...
    """
    vailed_Hand=dict(hand)
    if word not in wordList :
        return False
    else :
     for i in word : 
        if i not in vailed_Hand :
            return False
        else :
            vailed_Hand[i]=vailed_Hand[i]-1
            if vailed_Hand[i]==0 :
                del vailed_Hand[i]
    return True        
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    for x in range(n):
     ...
     while vailed : 
       if word=='.': 
         return 'Goodbye! Total score: ',sum ,'points'
     ...
     if calculateHandlen(hand)==0 :
        return 'Run out of letters. Total score:',sum,'points.'
     print('Current hand: ', end=' '); displayHand(hand)
    return sum     
